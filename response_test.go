...
func TestBodyPreservesExplicitCookieAttributes(t *testing.T) {
	...
	inst := &Instance{Config: Config{Domain: "example.com", HttpOnly: true, MaxAge: time.Hour}}
	ctx := &Context{
		Meta: infra.NewMeta(), inst: inst, writer: rec,
	...
	if defaults == nil || !defaults.HttpOnly || defaults.Path != "/" || defaults.Domain != "example.com" || defaults.MaxAge != int(time.Hour.Seconds()) {
		t.Fatalf("string cookie did not use instance defaults: %#v", defaults)
	}
}
